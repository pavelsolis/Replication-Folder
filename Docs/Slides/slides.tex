...
\begin{frame}[label=examples]
	...
	\begin{enumerate}[<\iftoggle{stops}{+-}{1}>]
		...
		\settoggle{coloreq}{true}							% Set toggle to highlight part of equation
		\input{../Equations/exEqU}
		\vspace{-1cm}
		\item<1->[] where \(\error\) (defined in \texttt{variables.tex}) is the error
		\item[\ding{43}] Reference example: \cite{ChangLi:2017AER}. % Set marker
		\setcounter{currentenumi}{\theenumi}				% Store actual item number
		\setcounter{enumi}{\thecurrentenumi+1}				% Use stored item number
		\item Cross-reference examples: to number in paper (\double{../Numbers/exNumMean.txt}) and to item \ref{item:eq} above
...
\begin{frame}[label=tableex]
	...
	\begin{tikzpicture}
	\node (table) at (0,0)
		{\includegraphics[trim={2.9cm 20.8cm 3cm 3cm},clip,height=5cm,width=\textwidth,keepaspectratio]{../Tables/extabfrag.pdf}};
		\uncover<\iftoggle{stops}{2-}{1}>{\draw[red,thick] (-1.2,-0.47) rectangle (0.9,0.57);}
